...
const unsigned char NHD_ICON_smiley20x20[] = {
...
void NHD_OLED_Command(NHD_OLED_HandleTypeDef* holed, uint8_t Value)
{
    NHD_OLED_CS_LOW();
    NHD_OLED_DC_LOW();
    NHD_OLED_DataWrite(holed, Value);
    NHD_OLED_CS_HIGH();
}
...
{
    NHD_OLED_Command(holed, 0x17);
    NHD_OLED_Data(holed, x_start);
    NHD_OLED_Command(holed, 0x18);
    NHD_OLED_Data(holed, x_end);
}

void NHD_OLED_SetRowAddress(NHD_OLED_HandleTypeDef* holed, uint8_t y_start, uint8_t y_end)
{
    NHD_OLED_Command(holed, 0x19);
    NHD_OLED_Data(holed, y_start);
    NHD_OLED_Command(holed, 0x1A);
    NHD_OLED_Data(holed, y_end);
}

void NHD_OLED_WriteMemoryStart(NHD_OLED_HandleTypeDef* holed)
{
    NHD_OLED_Command(holed, 0x22);
}

void NHD_OLED_Pixel(NHD_OLED_HandleTypeDef* holed, unsigned long color)    // write one pixel of a given color
{
    NHD_OLED_SerialPixelData(holed, (color>>16));
    NHD_OLED_SerialPixelData(holed, (color>>8));
    NHD_OLED_SerialPixelData(holed, color);
}
...
{
    NHD_OLED_Command(holed, 0x20);
    NHD_OLED_Data(holed, x_pos);
    NHD_OLED_Command(holed, 0x21);
    NHD_OLED_Data(holed, y_pos);
}

void NHD_OLED_FillScreen(NHD_OLED_HandleTypeDef* holed, unsigned long color)    // fill screen with a given color
{
    unsigned int i;
...
void NHD_OLED_Init(NHD_OLED_HandleTypeDef* holed)
{
    ...
    NHD_OLED_Command(holed, 0x04);
    NHD_OLED_Data(holed, 0x01);
    HAL_Delay(1);
    
     // normal mode
    NHD_OLED_Command(holed, 0x04); 
    NHD_OLED_Data(holed, 0x00); 
    HAL_Delay(1);
    
    // display off
    NHD_OLED_Command(holed, 0x06);
    NHD_OLED_Data(holed, 0x00);
    HAL_Delay(1);
    
    // turn on internal oscillator using external resistor
    NHD_OLED_Command(holed, 0x02);
    NHD_OLED_Data(holed, 0x01); 
    
    // 90 hz frame rate, divider 0
    NHD_OLED_Command(holed, 0x03);
    NHD_OLED_Data(holed, 0x30); 
    
    // duty cycle 127
    NHD_OLED_Command(holed, 0x28);
    NHD_OLED_Data(holed, 0x7F);
    
    // start on line 0
    NHD_OLED_Command(holed, 0x29);
    NHD_OLED_Data(holed, 0x00); 
    
    // rgb_if
    NHD_OLED_Command(holed, 0x14);
    NHD_OLED_Data(holed, 0x31); 
    
    // Set Memory Write Mode
    NHD_OLED_Command(holed, 0x16);
    NHD_OLED_Data(holed, 0x76);
    
    // driving current r g b (uA)
    NHD_OLED_Command(holed, 0x10);
    NHD_OLED_Data(holed, 0x45);
    NHD_OLED_Command(holed, 0x11);
    NHD_OLED_Data(holed, 0x34);
    NHD_OLED_Command(holed, 0x12);
    NHD_OLED_Data(holed, 0x33);
    
    // precharge time r g b
    NHD_OLED_Command(holed, 0x08);
    NHD_OLED_Data(holed, 0x04);
    NHD_OLED_Command(holed, 0x09);
    NHD_OLED_Data(holed, 0x05);
    NHD_OLED_Command(holed, 0x0A);
    NHD_OLED_Data(holed, 0x05);
    
    // precharge current r g b (uA)
    NHD_OLED_Command(holed, 0x0B);
    NHD_OLED_Data(holed, 0x9D);
    NHD_OLED_Command(holed, 0x0C);
    NHD_OLED_Data(holed, 0x8C);
    NHD_OLED_Command(holed, 0x0D);
    NHD_OLED_Data(holed, 0x57);
    
    // Set Reference Voltage Controlled by External Resister
    NHD_OLED_Command(holed, 0x80);
    NHD_OLED_Data(holed, 0x00);
    
    // mode set
    NHD_OLED_Command(holed, 0x13);
    NHD_OLED_Data(holed, 0xA0);
    
    NHD_OLED_SetColumnAddress(holed, 0, 159);
    NHD_OLED_SetRowAddress(holed, 0, 127);
...
void NHD_OLED_Text(NHD_OLED_HandleTypeDef* holed, unsigned char x_pos, unsigned char y_pos, unsigned char letter, unsigned long textColor, unsigned long backgroundColor)  // function to show text
{
    int i;
    int count;
    ...
    for(i=0;i<8;i++)     // each character is 8 pixels tall
    {
        ...
        for (count=0;count<5;count++)    // each character is 5 pixels wide
        {
            if((NHD_OLED_Ascii_1[letter][count] & mask) == mask)
                NHD_OLED_Pixel(holed,textColor);
            else
                NHD_OLED_Pixel(holed,backgroundColor);
        }
        y_pos++;
        mask = mask >> 1;
    }
...
void NHD_OLED_Text2x(NHD_OLED_HandleTypeDef* holed, unsigned char x_pos, unsigned char y_pos, unsigned char letter, unsigned long textColor, unsigned long backgroundColor)  // function to show text (2x size)
{
    int i;
    int count;
    unsigned char mask = 0x80;
...
void NHD_OLED_Icon(NHD_OLED_HandleTypeDef* holed, unsigned char x_pos, unsigned char y_pos)    // function to show icon
{
    unsigned int i, j;
    unsigned int incr = 0;
    ...
    for(i=0;i<20;i++)
    {
        NHD_OLED_SetPosition(holed,x_pos,y_pos+i);
        NHD_OLED_WriteMemoryStart(holed);
        for(j=0;j<60;j+=3)
        {
            NHD_OLED_SerialPixelData(holed,NHD_ICON_smiley20x20[(j+incr)]);
            NHD_OLED_SerialPixelData(holed,NHD_ICON_smiley20x20[((j+incr)+1)]);
            NHD_OLED_SerialPixelData(holed,NHD_ICON_smiley20x20[((j+incr)+2)]);
        }
        incr += 60;
    }
}

void NHD_OLED_Spectrum(NHD_OLED_HandleTypeDef* holed)                  // function to show color spectrum
{
    unsigned char i, j;
    ...
    for(i=0;i<160;i++)
    {
        NHD_OLED_Pixel(holed,WHITE);
    }
    NHD_OLED_SetPosition(holed,0,1);
    NHD_OLED_WriteMemoryStart(holed);
    for(i=0;i<160;i++)
    {
        NHD_OLED_Pixel(holed,WHITE);
    ...
    for(i=0;i<40;i++)
    {
        ...
        green = 0x00;
        red = 0xFF;
        NHD_OLED_SetPosition(holed,0,(2+i));
        NHD_OLED_WriteMemoryStart(holed);
        NHD_OLED_Pixel(holed,WHITE);
        NHD_OLED_Pixel(holed,WHITE);
        for(j=0;j<26;j++)
        {
            NHD_OLED_Data(holed,blue);
            NHD_OLED_Data(holed,green);
            NHD_OLED_Data(holed,red);
            green += 9;
        }
        for(j=0;j<26;j++)
        {
    ...
    NHD_OLED_SetPosition(holed,0,42);
    NHD_OLED_WriteMemoryStart(holed);
    for(i=0;i<160;i++)
    {
        NHD_OLED_Pixel(holed,WHITE);
    }
    NHD_OLED_SetPosition(holed,0,43);
    NHD_OLED_WriteMemoryStart(holed);
    for(i=0;i<160;i++)
    {
        NHD_OLED_Pixel(holed,WHITE);
